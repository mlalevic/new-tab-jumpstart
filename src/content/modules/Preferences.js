...
function(aObserver) {
  let prefFullName = this._prefBranch.substring(0, this._prefBranch.length - 1);

  try {
    this._prefSvc.addObserver(prefFullName, aObserver, false);
  }
  catch (ex) {}
};
...
function(aObserver) {
  ...
  try {
    this._prefSvc.removeObserver(prefFullName, aObserver);
  }
  catch (ex) {}
};